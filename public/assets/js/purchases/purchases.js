...
export class PurchaseModule {
    ...
    renderBaseStructure() {
        return `
            <div class="purchases-layout">
                ...
                <div id="create-purchase-modal" class="modal-overlay hidden" style="display:none; z-index:1000;">
                    <div class="modal-content" style="width: 95%; max-width: 2000px; height: 85vh;">
                        ...
                        <div class="purchase-modal-body" id="purchase-modal-body">
                            <div class="purchase-col" id="step-purch-provider" style="width: 25%;"><h4>1. Proveedor</h4><input type="text" style="display:none" aria-hidden="true"><input type="search" id="purch-search-provider" name="pur_prov" class="rustic-input" placeholder="Buscar..." style="margin-bottom:10px;"><div id="purch-providers-list" class="resource-list" style="max-height: 300px; overflow-y: auto;"></div><div id="selected-provider-display" style="margin-top:10px; padding:10px; background:var(--accent-color-quat-opacity); border-radius:4px; font-size:0.9rem; color:var(--accent-color); display:none;"></div></div>
                            <div class="purchase-col" id="step-purch-products" style="width: 45%;"><h4>2. Productos (Costo)</h4><input type="text" style="display:none" aria-hidden="true"><input type="search" id="purch-search-product" name="pur_prod" class="rustic-input" placeholder="Buscar..." style="margin-bottom:10px;"><div id="purch-products-list" class="resource-list" style="flex:1; overflow-y:auto;"></div></div>
                        ...
                        <div id="mobile-purch-bar" class="hidden-desktop" style="display:none; position:relative; width:100%; background:white; padding:15px; padding-bottom:calc(15px + env(safe-area-inset-bottom, 0px)); border-top:1px solid #ccc; box-shadow:0 -5px 15px rgba(0,0,0,0.1); z-index:99999; box-sizing:border-box;">
                ...
                <div id="quick-expense-modal" class="modal-overlay hidden" style="align-items:flex-end; display:none; z-index:10000;"><div class="m-expense-sheet"><div class="m-expense-handle"></div><div class="m-expense-header"><div class="m-expense-header-inner"><div class="m-expense-icon-wrap"><i id="quick-modal-icon" class="ph-bold ph-lightning"></i></div><div><div class="m-expense-title" id="quick-modal-title">Gasto Rápido</div><div class="m-expense-subtitle">Registrá un gasto rápidamente</div></div></div><button class="m-expense-close-btn" id="close-quick-modal" type="button"><i class="ph-bold ph-x"></i></button></div><div class="m-expense-body"><form id="quick-expense-form"><div class="m-expense-amount-wrap"><div class="m-expense-amount-label">Monto Total</div><div class="m-expense-amount-row"><span class="m-expense-currency">$</span><input type="number" id="quick-amount" class="m-expense-amount-input" placeholder="0,00" step="0.01" required></div></div><div class="m-expense-row-2"><div class="m-expense-field" style="margin-bottom:0;"><label class="m-expense-label"><i class="ph ph-calendar"></i> Fecha</label><input type="date" id="quick-date" class="m-expense-input"></div><div class="m-expense-field" style="margin-bottom:0;"><label class="m-expense-label"><i class="ph ph-tag"></i> Categoría</label><select id="quick-category" class="m-expense-input m-expense-select"><option value="General">General</option><option value="Servicios">Servicios</option><option value="Viáticos">Viáticos</option><option value="Alquiler">Alquiler</option><option value="Mantenimiento">Mantenimiento</option><option value="Otros">Otros</option></select></div></div><div class="m-expense-field"><label class="m-expense-label"><i class="ph ph-building-office"></i> Proveedor</label><select id="quick-provider" class="m-expense-input m-expense-select"><option value="">Desconocido / Ninguno / Vario</option></select></div><div class="m-expense-field"><label class="m-expense-label"><i class="ph ph-note"></i> Nota</label><textarea id="quick-note" class="m-expense-input m-expense-textarea" placeholder="Descripción opcional..."></textarea></div><button type="submit" id="submit-quick-btn" class="m-expense-submit-btn"><i class="ph-bold ph-check"></i> Registrar Gasto</button></form></div></div></div>
                <div id="detail-purchase-modal" class="modal-overlay hidden" style="align-items:center; justify-content:center; display:none; z-index:20000; padding: 20px;" onclick="if(event.target === this) window.purchasesModule.closeModal('detail-purchase-modal')"><div class="modal-content" style="width: 500px; max-width: 100%; background: #fff; border-radius: 0 !important; margin: auto; box-shadow: 0 10px 30px rgba(0,0,0,0.2);"><div class="modal-header" style="padding: 15px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center;"><h3 style="margin: 0; font-size: 1.2rem;">Detalle de Movimiento</h3><button class="modal-close-btn" id="close-detail-purch-modal" onclick="window.purchasesModule.closeModal('detail-purchase-modal')">&times;</button></div><div class="modal-body ticket-scroll" id="detail-purch-content" style="padding: 20px; overflow-y: auto; max-height: calc(100vh - 120px);"><div class="purchase-modal-body"></div></div></div></div>
            </div>
        `;
    }
    ...
    closeModal(id) {
        ...
        if (!el) return;
        if (id === 'quick-expense-modal') {
            const sheet = el.querySelector('.m-expense-sheet');
            if (sheet) {
    ...
    async openQuickModal(d) { if (!this.availableProviders.length) await this.fetchResources(); this.fillProviderSelect('quick-provider'); const m = document.getElementById('quick-expense-modal'), f = document.getElementById('quick-expense-form'), t = document.getElementById('quick-modal-title'), b = document.getElementById('submit-quick-btn'), ic = document.getElementById('quick-modal-icon'); f.reset(); document.getElementById('quick-date').valueAsDate = new Date; if (d) { this.editingId = d.id; t.textContent = 'Editar Gasto'; if (ic) { ic.className = 'ph ph-pencil-simple'; } b.textContent = "Actualizar Gasto"; document.getElementById('quick-amount').value = d.total; document.getElementById('quick-provider').value = d.provider_id || ""; document.getElementById('quick-category').value = d.category || "General"; document.getElementById('quick-note').value = d.notes || ""; if (d.created_at) document.getElementById('quick-date').value = d.created_at.split(' ')[0] } else { this.editingId = null; t.textContent = 'Gasto Rápido'; if (ic) { ic.className = 'ph-bold ph-lightning'; } b.innerHTML = '<i class="ph-bold ph-check"></i> Registrar Gasto'; } m.classList.remove('hidden'); m.style.display = 'flex'; document.getElementById('quick-amount').focus() }
    async submitQuickExpense(e) { e.preventDefault(); const b = document.getElementById('submit-quick-btn'); b.disabled = true; const ot = b.innerHTML; b.innerHTML = '<i class="ph ph-circle-notch"></i> Procesando...'; const a = parseFloat(document.getElementById('quick-amount').value); if (isNaN(a) || a <= 0) { pop_ups.warning("Monto inválido"); b.disabled = false; b.innerHTML = ot; return } const p = { id: this.editingId, total: a, provider_id: document.getElementById('quick-provider').value || null, category: document.getElementById('quick-category').value || 'General', notes: document.getElementById('quick-note').value || null, created_at: document.getElementById('quick-date').value + ' ' + new Date().toLocaleTimeString(), items: [] }; const ep = this.editingId ? '/api/purchases/update.php' : '/api/purchases/create.php'; try { const r = await this.sendRequest(ep, p); if (r.success) { this.closeModal('quick-expense-modal'); this.loadHistory(this.currentSortOrder); pop_ups.info(this.editingId ? 'Actualizado' : 'Registrado') } else pop_ups.error(r.message) } catch (e) { pop_ups.error("Error de conexión") } b.disabled = false; b.innerHTML = ot }
    async deletePurchase(i) { if (!await pop_ups.confirm("Eliminar", "¿Seguro?")) return; try { const r = await this.sendRequest('/api/purchases/delete.php', { id: i }); if (r.success) { pop_ups.info("Eliminado"); this.loadHistory(this.currentSortOrder) } else pop_ups.error(r.message || "No se pudo eliminar el registro.") } catch (e) { pop_ups.error("Error de conexión al eliminar.") } }
    ...
    async showDetails(id) {
        if (!document.getElementById('detail-purchase-modal')) {
            this.init();
        }
        try {
            const res = await api.getPurchaseDetails(id);
