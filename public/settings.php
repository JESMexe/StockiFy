...
<head>
    ...
    <script type="module" src="/assets/js/payments/payments.js"></script>
</head>
...
if ($canConfigRemito) {
    ...
    if ($invSettings) {
        ...
        // Obtener columnas seguras del inventario
        $stmtTable = $dbInstance->prepare("SELECT table_name FROM user_tables WHERE inventory_id = ? LIMIT 1");
        $stmtTable->execute([$activeInventoryId]);
        $tableName = $stmtTable->fetchColumn();

        if ($tableName) {
            $stmtCols = $dbInstance->prepare(
            ...
            $excludedCols = ['id', 'public_visible', 'buy_price', 'hard_gain', 'percentage_gain', 'receipt_price', 'min_stock', 'created_at', 'updated_at', 'inventory_id', 'user_id'];
            $safeColumns = array_filter(
                $allDbColumns,
                fn($col) =>
                !in_array(strtolower($col), $excludedCols)
            );
        }
    }
}
...
<body id="page-configuration">
    ...
    <main class="text-left">
        <div class="config-layout-wrapper">
            <div id="options-config-container">
                ...
                <?php endif; ?>
                <div class="btn" id="btn-config-suscripcion">
                    <p><i class="ph ph-crown"></i> Mi Suscripción</p>
                </div>
            ...
            <div id="config-container">
                    ...
                    <form class="flex-column" id="form-micuenta">
                        ...
                        <div class="config-grid">
                            <div class="rustic-block">
                                ...
                                <p class="unsaved-hint hidden"
                                    style="color: var(--accent-color); font-size: 0.8rem; margin-top: 4px; font-weight: 500;">
                                    Recordá guardar los cambios</p>
                            </div>
                            <div class="rustic-block">
                    ...
                    <div id="remito-container" class="hidden">
                        <form class="flex-column" id="form-remito" enctype="multipart/form-data">
                            ...
                            <p style="color: #64748b; font-size: 0.9rem; margin-bottom: 1.5rem;">
                                Personalizá la cabecera de tus remitos con los datos de tu negocio. Si no colocás nada,
                                estos campos aparecerán vacíos en el remito.
                            </p>

                            <div class="config-grid">
                                <div class="rustic-block"
                                    style="grid-column: 1 / -1; display: flex; flex-direction: column; gap: 10px;">
                                    <label class="option-label">Logo del Negocio</label>
                                    <div style="display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
                                        <div id="remito-logo-preview-container"
                                            style="width: 150px; height: 80px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; background: #fafafa; border-radius: 8px; overflow: hidden; position: relative;">
                                            <img id="remito-logo-preview" src="" alt="Vista previa"
                                                style="max-width: 100%; max-height: 100%; object-fit: contain; display: none;">
                                            <span id="remito-logo-placeholder"
                                                style="color: #aaa; font-size: 0.8rem; text-align: center; padding: 5px;">Sin
                                                Logo</span>
                                        </div>
                                        <div style="display: flex; flex-direction: column; gap: 8px;">
                                            <input type="file" id="remito_logo_input" name="remito_logo" accept="image/*"
                                                style="display: none;">
                                            <button type="button" class="btn btn-secondary"
                                                onclick="document.getElementById('remito_logo_input').click();"
                                                style="margin:0; width: auto; font-size: 0.85rem; padding: 8px 16px;">Seleccionar
                                                Imagen</button>
                                            <button type="button" class="btn btn-secondary" id="btn-delete-logo"
                                                style="margin:0; width: auto; font-size: 0.85rem; padding: 8px 16px; color: var(--accent-red); border-color: var(--accent-red); display: none;">Eliminar
                                                Logo</button>
                                        </div>
                                    </div>
                                    <p style="font-size: 0.75rem; color: #666; margin: 0;">Formatos recomendados: PNG, JPG,
                                        JPEG, WEBP. Relación de aspecto apaisada (ej. 2:1 o 3:1).</p>
                                </div>

                                <div class="rustic-block" style="grid-column: 1 / -1;">
                                    <label class="option-label" for="remito_description">Descripción del Negocio /
                                        Leyenda</label>
                                    <input class="config-input" type="text" id="remito_description"
                                        name="remito_description" placeholder="Ej: Venta de indumentaria por mayor y menor."
                                        autocomplete="off">
                                    <p class="unsaved-hint hidden"
                                        style="color: var(--accent-color); font-size: 0.8rem; margin-top: 4px; font-weight: 500;">
                                        Recordá guardar los cambios</p>
                                ...
                                <div class="rustic-block" style="grid-column: 1 / -1;">
                                    ...
                                    <input class="config-input" type="text" id="remito_url" name="remito_url"
                                        placeholder="Ej: https://www.minegocio.com" autocomplete="off">
                                    <p class="unsaved-hint hidden"
                                        style="color: var(--accent-color); font-size: 0.8rem; margin-top: 4px; font-weight: 500;">
                                        Recordá guardar los cambios</p>
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary" id="btn-guardar-remito" disabled
                                style="margin-top: 1.5rem;">Guardar Configuración de Remito</button>
                        </form>
                    </div>
                <?php endif; ?>
                ...
                <?php if ($canConfigRemito): ?>
                    <div id="catalogo-container" class="hidden">
                        ...
                        <p style="color: #64748b; font-size: 0.9rem; margin-bottom: 1.5rem;">
                            Publicá tus productos en internet. Compartí el link por WhatsApp o Instagram y tus clientes
                            podrán ver tu catálogo sin necesidad de iniciar sesión.
                        </p>
                        ...
                        <form class="flex-column" id="form-catalogo">
                            ...
                            <div class="catalog-row-block">
                                <div>
                                    ...
                                    <p>Cuando está activo, cualquier persona con el link puede ver tus productos publicados.
                                    </p>
                                </div>
                            ...
                            <div id="catalog-settings-body"
                                style="<?php echo $catalogActive ? '' : 'opacity:0.5; pointer-events:none;'; ?>">

                                <!-- Slug / URL -->
                                <div class="rustic-block" style="margin-top:1.5rem;">
                                    <label class="option-label" for="catalog_slug">URL del Catálogo <span
                                            class="helper-tag">Identificador único</span></label>
                                    <div class="slug-input-wrapper">
                                        ...
                                        <div style="flex:1; position:relative; height:100%;">
                                            <input type="text" id="catalog_slug"
                                                ...
                                                placeholder="mi-negocio" autocomplete="off" pattern="[a-z0-9\-]+"
                                                style="padding-right:2.5rem;">
                                            <span id="slug-status-icon"
                                                style="position:absolute; right:10px; top:50%; transform:translateY(-50%); font-size:1.1rem;"></span>
                                        </div>
                                    </div>
                                    <p id="slug-feedback"
                                        style="font-size:0.8rem; margin-top:4px; min-height:1em; margin-bottom:0;"></p>
                                    <div id="catalog-url-preview" class="catalog-url-card"
                                        style="display:<?php echo $catalogSlug ? 'flex' : 'none'; ?>;">
                                        <i class="ph ph-link"></i>
                                        <a id="catalog-url-link"
                                            href="/catalogo/<?php echo htmlspecialchars($catalogSlug); ?>" target="_blank">
                                            stockify.com.ar/catalogo/<?php echo htmlspecialchars($catalogSlug); ?>
                                        </a>
                                        <button type="button" id="btn-copy-catalog-url" class="btn btn-secondary"
                                            style="margin:0; padding:6px 16px; font-size:0.75rem; height:auto;">
                                ...
                                <div style="margin-top: 1.5rem;">
                                    <button type="button" id="btn-open-catalog-customizer"
                                        class="btn-catalog-customizer-trigger">
                            ...
                            <button type="submit" class="btn btn-primary" id="btn-guardar-catalogo"
                ...
                <div id="suscripcion-container" class="hidden">
                    <!-- Se renderiza dinámicamente por payments.js -->
                ...
                <div id="soporte-container" class="hidden">
                    <div style="text-align: center; padding: 2rem;">
                        ...
                        <div
                            ...
                            <a href="mailto:soporte@stockify.com.ar" class="btn btn-primary"
                                style="margin: 0; display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; width: auto; min-width: 200px;"><i
                                    class="ph ph-envelope-simple"></i> Vía Email</a>
                            <a href="https://wa.me/5491163642040" target="_blank" class="btn"
                                style="margin: 0; display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem; width: auto; min-width: 200px; background-color: var(--accent-green); color: #000; border: 2px solid #000;"><i
                                    class="ph ph-whatsapp-logo" style="font-size: 1.2rem;"></i> Vía WhatsApp</a>
                        </div>
                ...
                <div class="catalog-customizer-inner">
                    ...
                    <div class="customizer-panel-body">
                        ...
                        <div class="customizer-controls">
                            ...
                            <div class="customizer-section">
                                ...
                                <div class="rustic-block" style="display: flex; flex-direction: column; gap: 10px;">
                                    <div style="display: flex; gap: 16px; align-items: center; flex-wrap: wrap;">
                                        <div id="catalog-logo-preview-container"
                                            style="width: 80px; height: 80px; border: 2px dashed #ccc; display: flex; align-items: center; justify-content: center; background: #fafafa; border-radius: 50%; overflow: hidden; flex-shrink:0;">
                                            <img id="catalog-logo-preview"
                                                src="<?php echo htmlspecialchars($catalogSettings['logo_url'] ?? ''); ?>"
                                                alt="Vista previa"
                                                style="width: 100%; height: 100%; object-fit: cover; display: <?php echo !empty($catalogSettings['logo_url']) ? 'block' : 'none'; ?>;">
                                            <span id="catalog-logo-placeholder"
                                                style="color: #aaa; font-size: 0.7rem; text-align: center; padding: 4px; display: <?php echo empty($catalogSettings['logo_url']) ? 'block' : 'none'; ?>;">Sin
                                                logo</span>
                                        </div>
                                        <div style="display: flex; flex-direction: column; gap: 8px;">
                                            <input type="file" id="catalog_logo_input" accept="image/*"
                                                style="display: none;">
                                            <input type="hidden" id="catalog_logo_url"
                                                value="<?php echo htmlspecialchars($catalogSettings['logo_url'] ?? ''); ?>">
                                            <button type="button" class="btn btn-secondary"
                                                onclick="document.getElementById('catalog_logo_input').click();"
                                                style="margin:0; width: auto; font-size: 0.8rem; padding: 6px 14px;">Seleccionar
                                                Imagen</button>
                                            <button type="button" class="btn btn-secondary" id="btn-delete-catalog-logo"
                                                style="margin:0; width: auto; font-size: 0.8rem; padding: 6px 14px; color: var(--accent-red); border-color: var(--accent-red); display: <?php echo !empty($catalogSettings['logo_url']) ? 'block' : 'none'; ?>;">Eliminar</button>
                                        </div>
                                    </div>
                                    <p style="font-size: 0.72rem; color: #666; margin: 0;">PNG, JPG, WEBP. Si no subís
                                        imagen, se mostrará la inicial del negocio.</p>
                                </div>
                            </div>

                            <!-- Información de Contacto -->
                            <div class="customizer-section">
                                <h4 class="customizer-section-title"><i class="ph ph-address-book"></i> Contacto Público
                                </h4>
                                <div class="config-grid"
                                    style="grid-template-columns:1fr 1fr; gap:0.75rem; margin-bottom:0;">
                                    <div class="rustic-block">
                                        <label class="option-label" for="catalog_whatsapp"><i
                                                class="ph ph-whatsapp-logo"></i> WhatsApp</label>
                                    ...
                                    <div class="rustic-block">
                                        <label class="option-label" for="catalog_instagram"><i
                                                class="ph ph-instagram-logo"></i> Instagram</label>
                                    ...
                                    <div class="rustic-block" style="grid-column: 1 / -1;">
                                        <label class="option-label" for="catalog_address"><i class="ph ph-map-pin"></i>
                                            Dirección</label>
                            ...
                            <div class="customizer-section">
                                <h4 class="customizer-section-title"><i class="ph ph-list-plus"></i> Columnas Adicionales
                                </h4>
                                <p style="font-size: 0.75rem; color: #64748b; margin-bottom: 0.75rem;">
                                    Seleccioná hasta 3 columnas de tu inventario para mostrar como detalles adicionales al
                                    hacer clic en un producto.
                                </p>
                                <div style="display:flex; flex-direction:column; gap:0.75rem;">
                                    <div class="rustic-block">
                                        <label class="option-label" for="catalog_extra_column_1">Columna Extra 1</label>
                                        <select class="config-input" id="catalog_extra_column_1" style="margin-bottom:0;">
                                            <option value="">(Ninguna)</option>
                                            <?php foreach ($safeColumns as $col): ?>
                                                <option value="<?php echo htmlspecialchars($col); ?>" <?php echo ($catalogSettings['extra_column_1'] ?? '') === $col ? 'selected' : ''; ?>>
                                    ...
                                    <div class="rustic-block">
                                        <label class="option-label" for="catalog_extra_column_2">Columna Extra 2</label>
                                        <select class="config-input" id="catalog_extra_column_2" style="margin-bottom:0;">
                                            <option value="">(Ninguna)</option>
                                            <?php foreach ($safeColumns as $col): ?>
                                                <option value="<?php echo htmlspecialchars($col); ?>" <?php echo ($catalogSettings['extra_column_2'] ?? '') === $col ? 'selected' : ''; ?>>
                                                    <?php echo htmlspecialchars($col); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="rustic-block">
                                        <label class="option-label" for="catalog_extra_column_3">Columna Extra 3</label>
                                        <select class="config-input" id="catalog_extra_column_3" style="margin-bottom:0;">
                                            <option value="">(Ninguna)</option>
                                            <?php foreach ($safeColumns as $col): ?>
                                                <option value="<?php echo htmlspecialchars($col); ?>" <?php echo ($catalogSettings['extra_column_3'] ?? '') === $col ? 'selected' : ''; ?>>
                                                    <?php echo htmlspecialchars($col); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <div class="config-grid"
                                    ...
                                    <div class="rustic-block">
                                        ...
                                        <select class="config-input" id="catalog_theme_color" style="margin-bottom:0;">
                                            <option value="accent-color" <?php echo ($catalogSettings['theme_color'] ?? 'accent-color') === 'accent-color' ? 'selected' : ''; ?>>Aleatorio StockiFy
                                            </option>
                                    ...
                                    <div class="rustic-block" style="grid-column: 1 / -1; margin-top: 0.5rem;">
                                        <label class="option-label" for="catalog_font_family"><i class="ph ph-text-t"></i>
                                            Tipografía General</label>
                                        <select class="config-input" id="catalog_font_family" style="margin-bottom:0;">
                                            <option value="Outfit" <?php echo ($catalogSettings['font_family'] ?? 'Outfit') === 'Outfit' ? 'selected' : ''; ?>>Outfit (Moderna, geométrica)
                                            </option>
                                            ...
                                            <option value="Space Grotesk" <?php echo ($catalogSettings['font_family'] ?? '') === 'Space Grotesk' ? 'selected' : ''; ?>>Space Grotesk (Tech,
                                                neobrutalista)</option>
                                            <option value="Syne" <?php echo ($catalogSettings['font_family'] ?? '') === 'Syne' ? 'selected' : ''; ?>>Syne (Artística, expresiva)</option>
                                            <option value="Poppins" <?php echo ($catalogSettings['font_family'] ?? '') === 'Poppins' ? 'selected' : ''; ?>>Poppins (Redondeada, amigable)
                                            </option>
                                            <option value="Montserrat" <?php echo ($catalogSettings['font_family'] ?? '') === 'Montserrat' ? 'selected' : ''; ?>>Montserrat (Llamativa, clásica)
                                            </option>
                                            <option value="Playfair Display" <?php echo ($catalogSettings['font_family'] ?? '') === 'Playfair Display' ? 'selected' : ''; ?>>Playfair Display (Elegante,
                                                luxury)</option>
                                            <option value="Courier Prime" <?php echo ($catalogSettings['font_family'] ?? '') === 'Courier Prime' ? 'selected' : ''; ?>>Courier Prime (Monospace, retro)
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="config-grid"
                                    ...
                                    <div class="rustic-block">
                                        ...
                                        <select class="config-input" id="catalog_button_icon" style="margin-bottom:0;">
                                            <option value="ph-whatsapp-logo" <?php echo ($catalogSettings['button_icon'] ?? 'ph-whatsapp-logo') === 'ph-whatsapp-logo' ? 'selected' : ''; ?>>WhatsApp
                                            </option>
                                    ...
                                    <div class="rustic-block">
                                        ...
                                        <select class="config-input" id="catalog_button_color" style="margin-bottom:0;">
                                            <option value="whatsapp-green" <?php echo ($catalogSettings['button_color'] ?? 'whatsapp-green') === 'whatsapp-green' ? 'selected' : ''; ?>>Verde WhatsApp
                                            </option>
                                    ...
                                    <div class="rustic-block" style="grid-column: 1 / -1;">
                                        <label class="option-label" for="catalog_button_link">Enlace personalizado
                                            (opcional)</label>
                                        <input class="config-input" type="text" id="catalog_button_link"
                                            value="<?php echo htmlspecialchars($catalogSettings['button_link'] ?? ''); ?>"
                                            placeholder="https://wa.me/... (dejá vacío para el default)"
                                ...
                                <div style="display:flex; flex-direction:column; gap:0.6rem;">
                                    ...
                                    <div class="config-grid"
                                        ...
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_bg"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#F4F4F6" <?php echo ($catalogSettings['color_bg'] ?? '#F4F4F6') === '#F4F4F6' ? 'selected' : ''; ?>>Gris claro (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $cbg = $catalogSettings['color_bg'] ?? '#F4F4F6';
                                                    echo !in_array($cbg, ['#F4F4F6', '#FFFFFF', '#1A1A1A', '#EDF2F7', '#FFF9F0']) ? 'selected' : ''; ?>>Personalizado...
                                                    </option>
                                                </select>
                                                <input type="color" id="catalog_color_bg_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_bg'] ?? '#F4F4F6'); ?>"
                                        ...
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_pattern"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="rgba(0,0,0,0.08)" <?php echo ($catalogSettings['color_pattern'] ?? 'rgba(0,0,0,0.08)') === 'rgba(0,0,0,0.08)' ? 'selected' : ''; ?>>Negro
                                                        suave (default)</option>
                                                    ...
                                                    <option value="custom" <?php $cpat = $catalogSettings['color_pattern'] ?? 'rgba(0,0,0,0.08)';
                                                    echo !in_array($cpat, ['rgba(0,0,0,0.08)', 'rgba(0,0,0,0.2)', 'rgba(255,255,255,0.3)', 'rgba(163,190,140,0.4)', 'rgba(136,192,208,0.4)', 'rgba(235,203,139,0.4)']) ? 'selected' : ''; ?>>Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_pattern_custom"
                                                    value="<?php $cpat = $catalogSettings['color_pattern'] ?? 'rgba(0,0,0,0.08)';
                                                    echo str_starts_with($cpat, '#') ? htmlspecialchars($cpat) : '#8A8A8A'; ?>" class="custom-color-picker">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="config-grid"
                                        ...
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_card"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#FFFFFF" <?php echo ($catalogSettings['color_card'] ?? '#FFFFFF') === '#FFFFFF' ? 'selected' : ''; ?>>Blanco (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $ccard = $catalogSettings['color_card'] ?? '#FFFFFF';
                                                    echo !in_array($ccard, ['#FFFFFF', '#F4F4F6', '#FFF9F0', '#1A1A1A']) ? 'selected' : ''; ?>>
                                                        Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_card_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_card'] ?? '#FFFFFF'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                        <div class="rustic-block">
                                            <label class="option-label" for="catalog_color_accent">Acento (pills,
                                                logo)</label>
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_accent"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="theme" <?php echo ($catalogSettings['color_accent'] ?? 'theme') === 'theme' ? 'selected' : ''; ?>>Desde color principal
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $cacc = $catalogSettings['color_accent'] ?? 'theme';
                                                    echo !in_array($cacc, ['theme', '#A3BE8C', '#88C0D0', '#EBCB8B', '#BF616A', '#B48EAD', '#25D366']) ? 'selected' : ''; ?>>
                                                        Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_accent_custom"
                                                    value="<?php echo htmlspecialchars(($catalogSettings['color_accent'] ?? 'theme') === 'theme' ? '#88C0D0' : ($catalogSettings['color_accent'] ?? '#88C0D0')); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- Fila 3: Textos -->
                                    <div class="config-grid"
                                        style="grid-template-columns:1fr 1fr 1fr; gap:0.75rem; margin-bottom:0;">
                                        <div class="rustic-block">
                                            <label class="option-label" for="catalog_color_label">Etiqueta cat.</label>
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_label"
                                                    style="margin-bottom:0; flex:1;">
                                                    ...
                                                    <option value="custom" <?php $clab = $catalogSettings['color_label'] ?? '#8A8A8A';
                                                    echo !in_array($clab, ['#8A8A8A', '#1A1A1A', '#FFFFFF']) ? 'selected' : ''; ?>>Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_label_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_label'] ?? '#8A8A8A'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_title"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#1A1A1A" <?php echo ($catalogSettings['color_title'] ?? '#1A1A1A') === '#1A1A1A' ? 'selected' : ''; ?>>Negro (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $ctit = $catalogSettings['color_title'] ?? '#1A1A1A';
                                                    echo !in_array($ctit, ['#1A1A1A', '#FFFFFF', '#8A8A8A']) ? 'selected' : ''; ?>>Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_title_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_title'] ?? '#1A1A1A'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_price"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#1A1A1A" <?php echo ($catalogSettings['color_price'] ?? '#1A1A1A') === '#1A1A1A' ? 'selected' : ''; ?>>Negro (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $cpri = $catalogSettings['color_price'] ?? '#1A1A1A';
                                                    echo !in_array($cpri, ['#1A1A1A', '#FFFFFF', '#A3BE8C', '#BF616A']) ? 'selected' : ''; ?>>
                                                        Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_price_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_price'] ?? '#1A1A1A'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="config-grid"
                                        ...
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_header_bg"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#FFFFFF" <?php echo ($catalogSettings['color_header_bg'] ?? '#FFFFFF') === '#FFFFFF' ? 'selected' : ''; ?>>Blanco (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $chbg = $catalogSettings['color_header_bg'] ?? '#FFFFFF';
                                                    echo !in_array($chbg, ['#FFFFFF', '#F4F4F6', '#FFF9F0', '#1A1A1A']) ? 'selected' : ''; ?>>
                                                        Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_header_bg_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_header_bg'] ?? '#FFFFFF'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                        <div class="rustic-block">
                                            <label class="option-label" for="catalog_color_social_bg">Botones
                                                contacto</label>
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_social_bg"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#FFFFFF" <?php echo ($catalogSettings['color_social_bg'] ?? '#FFFFFF') === '#FFFFFF' ? 'selected' : ''; ?>>Blanco (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $csbg = $catalogSettings['color_social_bg'] ?? '#FFFFFF';
                                                    echo !in_array($csbg, ['#FFFFFF', '#F4F4F6', '#FFF9F0', '#1A1A1A']) ? 'selected' : ''; ?>>
                                                        Personalizado...</option>
                                                </select>
                                                <input type="color" id="catalog_color_social_bg_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_social_bg'] ?? '#FFFFFF'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                        <div class="rustic-block">
                                            ...
                                            <div style="display:flex; gap:4px; align-items:center;">
                                                <select class="config-input" id="catalog_color_badge_bg"
                                                    style="margin-bottom:0; flex:1;">
                                                    <option value="#A3BE8C" <?php echo ($catalogSettings['color_badge_bg'] ?? '#A3BE8C') === '#A3BE8C' ? 'selected' : ''; ?>>Verde (default)
                                                    </option>
                                                    ...
                                                    <option value="custom" <?php $cbbg = $catalogSettings['color_badge_bg'] ?? '#A3BE8C';
                                                    echo !in_array($cbbg, ['#A3BE8C', '#BF616A', '#88C0D0', '#EBCB8B', '#B48EAD']) ? 'selected' : ''; ?>>Personalizado...
                                                    </option>
                                                </select>
                                                <input type="color" id="catalog_color_badge_bg_custom"
                                                    value="<?php echo htmlspecialchars($catalogSettings['color_badge_bg'] ?? '#A3BE8C'); ?>"
                                                    class="custom-color-picker">
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="customizer-preview">
                            ...
                            <div class="mockup-zoom-controls">
                                <button type="button" class="mockup-zoom-btn" id="mockup-zoom-out" title="Alejar"><i
                                        class="ph ph-minus"></i></button>
                                <span class="mockup-zoom-label" id="mockup-zoom-label">100%</span>
                                <button type="button" class="mockup-zoom-btn" id="mockup-zoom-in" title="Acercar"><i
                                ...
                                <div class="catalog-mockup" id="catalog-mockup">
                                    ...
                                    <div class="mockup-nav" id="mockup-nav">
                                        <div class="mockup-nav-logo" id="mockup-nav-logo">
                                            <div class="mockup-logo-circle" id="mockup-logo-circle">
                                                <?php echo htmlspecialchars(!empty($inventoryName) ? mb_strtoupper(mb_substr($inventoryName, 0, 1)) : 'M'); ?>
                                            </div>
                                            <span class="mockup-business-name" id="mockup-business-name"
                                                data-original-name="<?php echo htmlspecialchars(!empty($inventoryName) ? $inventoryName : 'Mi Negocio'); ?>"><?php echo htmlspecialchars(!empty($inventoryName) ? $inventoryName : 'Mi Negocio'); ?></span>
                                        </div>
                                        <div class="mockup-nav-contacts" id="mockup-nav-contacts">
                                            <div class="mockup-contact-btn mockup-map" id="mockup-contact-map"
                                                style="display:none;"><i class="ph ph-map-pin"></i></div>
                                            <div class="mockup-contact-btn mockup-ig" id="mockup-contact-ig"
                                                style="display:none;"><i class="ph ph-instagram-logo"></i></div>
                                            <div class="mockup-contact-btn mockup-wa" id="mockup-contact-wa"
                                                style="display:none;"><i class="ph ph-whatsapp-logo"></i></div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="mockup-products-grid mockup-single">
                                            <div class="mockup-product-card">
                                                <div class="mockup-product-image-container">
                                                    ...
                                                    <img src="https://http2.mlstatic.com/D_Q_NP_2X_761181-MLA93322598447_092025-T.webp"
                                                        alt="Banqueta Alta Vittoria" class="mockup-product-img-real"
                                                        loading="lazy">
                                                </div>
                                                <div class="mockup-product-details">
                                                    <span class="mockup-product-category">General</span>
                                                    <h3 class="mockup-product-title">Banqueta Alta Vittoria Premium Negro
                                                    </h3>
    ...
    <script>
        ...
        <?php endif; ?>
        window.paymentData = {
            subscriptionActive: <?php echo (int)($currentUser['subscription_active'] ?? 0); ?>,
            autoDebitEnabled: <?php echo !empty($currentUser['mp_auto_debit_enabled']) ? 'true' : 'false'; ?>,
        };
    </script>
</body>
